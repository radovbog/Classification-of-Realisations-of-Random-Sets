...
#          else{
 #           N.dist.matrix_R <- readRDS(paste0(base_dir, radius, "_updated/", n_run, "_run_Ndist_200_real_r_", radius, "_R_", max_sample_size, ".rds"))
   ...
   #         w <- weights[n_run, which(sample_names %in% max_sample_size)]
    #        a <- normed[n_run, which(sample_names %in% max_sample_size)]
     #       
      ...
      ggsave(paste(base_dir, "Results_knn_", radius, "/", n_run,"_part_Hist_", num_real, "_", chars_names[[c]], "_", n_run, ".png", sep = ""),plot = last_plot(),width = 18,height = 7,units = "cm")
  ...
  ggsave(paste(base_dir, "Results_knn_", radius, "/", n_run, "_part_Box_", num_real, "_", num_runs, ".png", sep = ""),plot = last_plot(),width = 18,height = 7,units = "cm")
  write.csv(Misclassification_data, file=paste0(base_dir, "Results_knn_", radius, "/", n_run,"_part_Misclassification_data_", num_real, ".csv"))
